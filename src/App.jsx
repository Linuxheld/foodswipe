    ...
    try {
      ...
      const res = await fetch("https://api.anthropic.com/v1/messages", {
        ...
        body: JSON.stringify({
          ...
          messages: [{
            role: "user", content: `Übersetze dieses Rezept ins ${LANG_NAMES[lang] || "Deutsche"}. Antworte NUR mit JSON, keine Backticks:
...
- Koche-Fachbegriffe korrekt übersetzen`}]
  ...
  const buildDeck = useCallback((mood, allergens, seen) => {
    let list = allRecipes.filter(r => {
      ...
      if (mood === "Vegetarisch") return r.tags.some(tag => tag.toLowerCase().includes("vegetar"));
      if (mood === "Trending") return r.src === "YouTube" || r.src === "TheMealDB" || r.src === "TikTok Viral" || r.src === "Instagram" || r.tags.some(tag => tag === "Trending" || tag === "Viral" || tag === "TikTok");
      if (mood === "Wenig Kalorien") return r.cal < 420;
      if (mood === "Saisonal") { const mo = new Date().getMonth(); const season = [11, 0, 1].includes(mo) ? "winter" : [2, 3, 4].includes(mo) ? "spring" : [5, 6, 7].includes(mo) ? "summer" : "autumn"; const seasonTags = { winter: ["Winter", "Eintopf", "Suppe", "Braten"], spring: ["Frühling", "Spargel", "Salat", "Leicht", "Shakshuka", "Frühling"], summer: ["Sommer", "Grill", "Bowl", "Salat", "Erfrischend"], autumn: ["Herbst", "Kürbis", "Eintopf", "Pilze"] }[season]; return r.tags.some(tag => seasonTags.some(st => tag.toLowerCase().includes(st.toLowerCase()))); }
      return true;
    });
  ...
  // TheMealDB wird manuell über den Button in den Einstellungen geladen.
  // Kein Auto-Load beim Start (verhindert Produktions-Render-Probleme).
  ...
  // FONT wird nicht mehr als JSX gerendert – Google Fonts ist bereits in index.html geladen.
  const BG = ({ img }) => (
