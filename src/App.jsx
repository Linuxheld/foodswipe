...
const shuffle = arr => { const a=[...arr]; for(let i=a.length-1;i>0;i--){const j=Math.floor(Math.random()*(i+1));[a[i],a[j]]=[a[j],a[i]];} return a; 
  // Trending v12 additions
  dubai_crepes:   "https://images.unsplash.com/photo-1519676867240-f03562e64548?w=800&q=80",
  birria_tacos:   "https://images.unsplash.com/photo-1565299585323-38d6b0865b47?w=800&q=80",
  baked_feta:     "https://images.unsplash.com/photo-1473093295043-cdd812d0e601?w=800&q=80",
  butter_chicken: "https://images.unsplash.com/photo-1588166524941-3bf61a9c41db?w=800&q=80",
  korean_dog:     "https://images.unsplash.com/photo-1619740455993-9e612b1af08a?w=800&q=80",
  salmon_rice:    "https://images.unsplash.com/photo-1553621042-f6e147245754?w=800&q=80",
  chilli_beef:    "https://images.unsplash.com/photo-1547592180-85f173990554?w=800&q=80",
  dalgona:        "https://images.unsplash.com/photo-1461023058943-07fcbe16d735?w=800&q=80",
  corn_soup:      "https://images.unsplash.com/photo-1547592166-23ac45744acd?w=800&q=80",
  truffle_pasta:  "https://images.unsplash.com/photo-1612874742237-6526221588e3?w=800&q=80",
};
...
const getSeason = () => {
  ...
  if(m>=12||m<=2) return ["Winter","Eintopf","Suppe","Braten","Warm"];
  if(m>=3&&m<=5)  return ["Frühling","Spargel","Salat","Leicht","Vegetarisch"];
  if(m>=6&&m<=8)  return ["Sommer","Grill","Bowl","Salat","Leicht"];
  return ["Herbst","Kürbis","Eintopf","Pilze","Risotto"];
};
const SEASON_LABEL = () => {
  const m = new Date().getMonth()+1;
  if(m>=12||m<=2) return "❄️ Winter";
  if(m>=3&&m<=5)  return "🌸 Frühling";
  if(m>=6&&m<=8)  return "☀️ Sommer";
  return "🍂 Herbst";
};
...
const LOCAL_RECIPES=[
  ...
  R(51,"Dubai-Schokolade Crêpes","20 Min",680,12,82,34,["breakfast","snack"],["Trending","Süß","TikTok"],"TikTok","#ff0050",IMG.dubai_crepes,["Gluten","Milch","Nüsse"],
    ["Crêpe-Teig aus Mehl, Ei, Milch & Butter rühren.","Dünne Crêpes in beschichteter Pfanne backen.","Kadayıf-Fäden in Butter goldbraun rösten.","Pistaziencreme + Schokolade schmelzen, verrühren.","Füllung auf Crêpe, einrollen.","Mit Puderzucker & gehackten Pistazien servieren."],
    [{name:"Crêpe-Teig",amount:"200g"},{name:"Pistaziencreme",amount:"4 EL"},{name:"Dunkle Schokolade",amount:"100g"},{name:"Kadayıf-Fäden",amount:"50g"},{name:"Pistazien",amount:"30g"}]),
  R(52,"Birria Tacos","45 Min",620,38,52,26,["lunch","dinner"],["Trending","Mexiko","TikTok"],"TikTok","#ff0050",IMG.birria_tacos,["Gluten"],
    ["Rindfleisch mit Chili, Cumin & Oregano 3 Std schmoren.","Consoммé (Schmorfond) in flache Schüssel gießen.","Tortillas in Consommé tauchen, in Pfanne legen.","Zerzupftes Fleisch + Käse drauf, zuklappen.","Goldbraun braten bis Käse schmilzt.","Mit Consommé zum Dippen & Koriander servieren."],
    [{name:"Rinderbrust",amount:"500g"},{name:"Mais-Tortillas",amount:"8 Stück"},{name:"Oaxaca-Käse",amount:"150g"},{name:"Guajillo-Chilis",amount:"4 Stück"},{name:"Koriander",amount:"1 Bund"}]),
  R(53,"Baked Feta Pasta","35 Min",540,18,72,22,["lunch","dinner"],["Trending","Vegetarisch","TikTok"],"TikTok","#ff0050",IMG.baked_feta,["Gluten","Milch"],
    ["Feta-Block in Auflaufform, Kirschtomaten drumherum.","Olivenöl, Knoblauch & Chili drüber.","Bei 200°C 30 Min backen bis Feta goldbraun.","Pasta al dente kochen, Nudelwasser aufheben.","Feta & Tomaten zerdrücken zu cremiger Sauce.","Pasta untermischen, mit Basilikum servieren."],
    [{name:"Feta Block",amount:"200g"},{name:"Kirschtomaten",amount:"400g"},{name:"Penne",amount:"250g"},{name:"Knoblauch",amount:"4 Zehen"},{name:"Olivenöl",amount:"4 EL"}]),
  R(54,"Butter Chicken","40 Min",580,44,32,28,["dinner"],["Indien","Trending","Hähnchen"],"TikTok","#ff0050",IMG.butter_chicken,["Milch"],
    ["Hähnchen in Joghurt + Gewürzen marinieren, anbraten.","Zwiebeln + Knoblauch + Ingwer goldbraun dünsten.","Tomatenpassata + Gewürze, 15 Min köcheln.","Butter + Sahne einrühren für cremige Sauce.","Hähnchen dazu, 10 Min ziehen lassen.","Mit Butter Naan & Basmati servieren."],
    [{name:"Hähnchenbrust",amount:"400g"},{name:"Tomatenpassata",amount:"400ml"},{name:"Sahne",amount:"150ml"},{name:"Butter",amount:"50g"},{name:"Garam Masala",amount:"2 EL"}]),
  R(55,"Korean Corn Dogs","25 Min",480,14,62,20,["snack"],["Korea","Trending","Fingerfood"],"TikTok","#ff0050",IMG.korean_dog,["Gluten","Ei","Milch"],
    ["Hotdog auf Stiel stecken, mit Mozzarella umwickeln.","Teig aus Mehl + Zucker + Ei + Milch rühren.","Stiel eintauchen, in Panko wälzen.","In 180°C Öl 4-5 Min goldbraun frittieren.","Noch heiß in Zucker wälzen.","Mit Ketchup + koreanischem Mayo servieren."],
    [{name:"Hotdogs",amount:"4 Stück"},{name:"Mozzarella",amount:"100g"},{name:"Tempura-Teig",amount:"150g"},{name:"Panko",amount:"80g"},{name:"Zucker",amount:"2 EL"}]),
  R(56,"TikTok Salmon Rice Bowl","15 Min",520,42,48,16,["lunch","dinner"],["TikTok","Trending","Fisch"],"TikTok","#ff0050",IMG.salmon_rice,["Fisch","Soja","Sesam"],
    ["Lachs mit Sojasoße + Sesamöl + Sriracha marinieren.","In Pfanne oder Luftfritteuse 8 Min knusprig garen.","Reis kochen, mit Reisessig würzen.","Avocado & Gurke schneiden.","Alles in Bowl anrichten.","Mit Sriracha-Mayo & Sesam toppen."],
    [{name:"Lachsfilet",amount:"200g"},{name:"Jasminreis",amount:"180g"},{name:"Avocado",amount:"1 Stück"},{name:"Sojasoße",amount:"2 EL"},{name:"Sriracha-Mayo",amount:"2 EL"}]),
  R(57,"Crispy Chilli Beef","30 Min",540,32,48,22,["lunch","dinner"],["China","Trending","Scharf"],"TikTok","#ff0050",IMG.chilli_beef,["Gluten","Soja"],
    ["Rindfleisch in dünne Streifen schneiden, in Stärke wenden.","In heißem Öl portionsweise knusprig frittieren.","Sauce: Sojasoße + Honig + Chilisauce + Reisessig.","Knoblauch + Chili in Pfanne anbraten.","Sauce dazu, aufkochen. Rindfleisch untermengen.","Mit Frühlingszwiebeln & Sesam über Reis servieren."],
    [{name:"Rindfleisch Streifen",amount:"300g"},{name:"Speisestärke",amount:"4 EL"},{name:"Chilisauce",amount:"3 EL"},{name:"Honig",amount:"2 EL"},{name:"Sojasoße",amount:"3 EL"}]),
  R(58,"Dalgona Coffee","5 Min",180,4,28,6,["breakfast","snack"],["Trending","Kaffee","TikTok"],"TikTok","#ff0050",IMG.dalgona,["Milch"],
    ["Instantkaffee + Zucker + heißes Wasser (2:2:2 EL).","3-4 Min schaumig aufschlagen bis cremig & fest.","Glas mit Eis füllen.","Kalte Milch drüber gießen.","Kaffee-Creme auf Milch setzen.","Mit Zimt bestäuben & gut umrühren beim Trinken!"],
    [{name:"Instantkaffee",amount:"2 EL"},{name:"Zucker",amount:"2 EL"},{name:"Heißes Wasser",amount:"2 EL"},{name:"Vollmilch",amount:"200ml"},{name:"Eis",amount:"nach Wunsch"}]),
  R(59,"Miso Ramen","35 Min",590,32,68,18,["dinner"],["Japan","Trending","Suppe"],"TikTok","#ff0050",IMG.ramen,["Gluten","Soja","Ei"],
    ["Brühe: Hühnerbrühe + Miso + Sojasoße + Sesamöl aufkochen.","Ramen-Nudeln kochen.","Eier wachsweich kochen (6,5 Min), in Sojasoße marinieren.","Chashu-Schwein in Scheiben schneiden.","Brühe in Schüssel, Nudeln einlegen.","Ei + Fleisch + Nori + Bambussprossen + Sesam obendrauf."],
    [{name:"Ramen-Nudeln",amount:"180g"},{name:"Miso-Paste",amount:"3 EL"},{name:"Hühnerbrühe",amount:"500ml"},{name:"Wachsweiches Ei",amount:"2 Stück"},{name:"Chashu",amount:"100g"}]),
  R(60,"Trüffel-Pasta","20 Min",620,18,72,28,["dinner"],["Trending","Pasta","Luxus"],"TikTok","#ff0050",IMG.truffle_pasta,["Gluten","Milch","Ei"],
    ["Pasta al dente kochen, Nudelwasser aufheben.","Butter in Pfanne schmelzen, Knoblauch dazu.","Parmesan + Nudelwasser zu cremiger Sauce rühren.","Trüffelöl einrühren – nicht mehr erhitzen!","Pasta dazu, gut schwenken.","Mit gehobeltem Parmesan + schwarzem Trüffel servieren."],
    [{name:"Tagliatelle",amount:"200g"},{name:"Parmesan",amount:"80g"},{name:"Trüffelöl",amount:"2 EL"},{name:"Butter",amount:"40g"},{name:"Knoblauch",amount:"2 Zehen"}]),
];
...
const MOODS=["Zur Uhrzeit","Alles","Schnell","High Protein","Vegetarisch","Trending","Saisonal","Wenig Kalorien"];
...
export default function FoodSwipe(){
  ...
  useEffect(()=>{save("fs_weekplan",weekPlan);},[weekPlan]);
  useEffect(()=>{save("fs_taste",tasteProfile);},[tasteProfile]);
  useEffect(()=>{save("fs_dailycal",dailyCal);},[dailyCal]);
  useEffect(()=>{save("fs_paar",paarLikes);},[paarLikes]);
  useEffect(()=>{save("fs_paaruser",paarUser);},[paarUser]);
  ...
  const buildDeck=useCallback((mood,allergens,seen)=>{
    let list=allRecipes.filter(r=>{
      ...
      if(mood==="Trending")return r.src==="YouTube"||r.src==="TikTok";
      if(mood==="Saisonal"){const tags=getSeason();return tags.some(t=>r.tags.some(rt=>rt.includes(t)));}
      if(mood==="Wenig Kalorien")return r.cal<420;
      return true;
    });
    const score=r=>(r.tags||[]).reduce((s,t)=>s+(tasteProfile[t]||0),0);
    const unseen=list.filter(r=>!seen.includes(String(r.id)));
    const seenL=list.filter(r=>seen.includes(String(r.id)));
    const sorted=[...unseen].sort((a,b)=>score(b)-score(a));
    return [...sorted,...shuffle(seenL)];
  },[allRecipes,slotType]);
  ...
  const [weekPlan,setWeekPlan]=useState(()=>load("fs_weekplan",{}));
  const [wpSlot,setWpSlot]=useState(null);
  const [tasteProfile,setTasteProfile]=useState(()=>load("fs_taste",{}));
  const [dailyCal,setDailyCal]=useState(()=>{
    const d=load("fs_dailycal",{date:"",total:0,goal:2000,log:[]});
    const today=new Date().toDateString();
    return d.date===today?d:{date:today,total:0,goal:2000,log:[]};
  });
  const [paarMode,setPaarMode]=useState(false);
  const [paarLikes,setPaarLikes]=useState(()=>load("fs_paar",[]));
  const [paarUser,setPaarUser]=useState(()=>load("fs_paaruser","A"));
  ...
  const doSwipe=dir=>{
    ...
    setTimeout(()=>{
      if(dir==="right"){
        ...
        // Lernender Algorithmus: Tag-Gewichte erhöhen
        setTasteProfile(tp=>{
          const np={...tp};
          (current.tags||[]).forEach(t=>{np[t]=(np[t]||0)+1;});
          return np;
        });
        // Paar-Modus: Match prüfen
        if(paarMode){
          const otherUser=paarUser==="A"?"B":"A";
          const otherLikes=load(`fs_paar_${otherUser}`,[]);
          const isMatch=otherLikes.some(x=>x.id===current.id);
          if(isMatch){showToast(`💞 MATCH! ${current.name}!`,"#ff6b9d");}
          else{
            const myKey=`fs_paar_${paarUser}`;
            save(myKey,[...load(myKey,[]).filter(x=>x.id!==current.id),current]);
            showToast(`❤️ ${paarUser}: ${current.name}`,"#ff6b35");
          }
        } else {
          showToast(`❤️ ${current.name} gespeichert!`);
        }
        // Kalorien-Tracking
        setDailyCal(dc=>({...dc,total:dc.total+(current.cal||0),log:[...dc.log,{name:current.name,cal:current.cal,time:new Date().toLocaleTimeString("de",{hour:"2-digit",minute:"2-digit"})}]}));
      }
  ...
  const BG=({img})=>(
    <div style={{position:"fixed",inset:0,zIndex:0}}>
      ...
      <div style={{position:"absolute",inset:0,backdropFilter:"blur(60px) saturate(1.8)",background:"linear-gradient(160deg,rgba(4,4,18,0.85) 0%,rgba(20,4,30,0.82) 50%,rgba(4,10,20,0.88) 100%)"}}/>
      <div style={{position:"absolute",inset:0,background:"radial-gradient(ellipse at 20% 20%,rgba(255,107,53,0.08) 0%,transparent 60%)",pointerEvents:"none"}}/>
      <div style={{position:"absolute",inset:0,background:"radial-gradient(ellipse at 80% 80%,rgba(138,43,226,0.06) 0%,transparent 60%)",pointerEvents:"none"}}/>
    </div>
  );
  const BackBtn=({to})=>(
  ...
  const ToastEl=()=>toast?(
    <div style={{position:"fixed",top:"1.2rem",left:"50%",transform:"translateX(-50%)",background:toast.color,color:"#000",padding:"0.65rem 1.6rem",borderRadius:"50px",fontSize:"0.85rem",fontWeight:"800",zIndex:9999,fontFamily:"'DM Sans',sans-serif",boxShadow:`0 8px 32px ${toast.color}55,0 2px 8px rgba(0,0,0,0.4)`,whiteSpace:"nowrap",backdropFilter:"blur(20px)",border:"1px solid rgba(255,255,255,0.2)"}}>
      {toast.msg}
    </div>
  ):null;
      ...
      <div style={{minHeight:"100dvh",fontFamily:"'DM Sans',sans-serif",color:"white",position:"relative"}}>
        ...
        <div style={{position:"relative",zIndex:1}}>
          ...
          <div style={{padding:"1rem 1.4rem"}}>
            ...
            {/* PORTIONS SELECTOR */}
            <div style={{display:"flex",alignItems:"center",gap:"0.6rem",marginBottom:"1rem",background:"rgba(255,255,255,0.06)",borderRadius:"14px",padding:"0.6rem 0.9rem",border:"1px solid rgba(255,255,255,0.08)"}}>
              <span style={{fontSize:"0.75rem",color:"rgba(255,255,255,0.45)",marginRight:"0.2rem"}}>👤 Portionen:</span>
              {[1,2,4,6].map(p=>(
                <button key={p} onClick={()=>setPortions(p)} style={{flex:1,background:portions===p?"linear-gradient(135deg,#ff6b35,#ff9a3c)":"rgba(255,255,255,0.08)",border:"none",borderRadius:"10px",color:"white",padding:"0.4rem 0",fontSize:"0.9rem",fontWeight:portions===p?"800":"400",cursor:"pointer",fontFamily:"inherit"}}>
                  {p}
                </button>
              ))}
            </div>
            ...
            {activeTab==="ingredients"&&(
              <div style={{background:"rgba(255,255,255,0.07)",borderRadius:"18px",padding:"1.1rem",marginBottom:"1rem",border:"1px solid rgba(255,255,255,0.1)"}}>
                {r.ingredients.map((ing,i)=>{
                  ...
                  const scaledAmt=ing.amount.replace(/(\d+\.?\d*)/g,n=>{
                    const scaled=parseFloat(n)*mult;
  ...
  if(screen==="settings")return(
    <div style={{minHeight:"100dvh",fontFamily:"'DM Sans',sans-serif",color:"white",position:"relative"}}>
      ...
      <div style={{position:"relative",zIndex:1,padding:"1.4rem"}}>
        ...
        {/* KALORIENZIEL HEUTE */}
        <div style={{background:"rgba(255,255,255,0.07)",borderRadius:"20px",padding:"1.1rem",marginBottom:"1rem",border:"1px solid rgba(255,255,255,0.08)",backdropFilter:"blur(20px)"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:"0.7rem"}}>
            <div>
              <div style={{fontSize:"0.62rem",letterSpacing:"2px",color:"rgba(255,107,53,0.9)"}}>📊 KALORIEN HEUTE</div>
              <div style={{fontSize:"1.8rem",fontWeight:"900",lineHeight:1.1,marginTop:"0.2rem"}}>{dailyCal.total} <span style={{fontSize:"0.9rem",fontWeight:"400",color:"rgba(255,255,255,0.4)"}}>/ {dailyCal.goal} kcal</span></div>
            </div>
            <div style={{textAlign:"right"}}>
              <div style={{fontSize:"0.72rem",color:"rgba(255,255,255,0.4)"}}>verbleibend</div>
              <div style={{fontSize:"1.2rem",fontWeight:"800",color:dailyCal.goal-dailyCal.total>0?"#4ade80":"#ff6b6b"}}>{Math.max(0,dailyCal.goal-dailyCal.total)}</div>
            </div>
          </div>
          {/* Progress Bar */}
          <div style={{background:"rgba(255,255,255,0.08)",borderRadius:"20px",height:"10px",overflow:"hidden",marginBottom:"0.8rem"}}>
            <div style={{width:`${Math.min(100,(dailyCal.total/dailyCal.goal)*100)}%`,height:"100%",background:dailyCal.total>dailyCal.goal?"linear-gradient(90deg,#ff4444,#ff6b6b)":"linear-gradient(90deg,#ff6b35,#ffcc02)",borderRadius:"20px",transition:"width 0.5s ease"}}/>
          </div>
          {/* Goal editor */}
          <div style={{display:"flex",alignItems:"center",gap:"0.5rem",marginBottom:"0.6rem"}}>
            <span style={{fontSize:"0.72rem",color:"rgba(255,255,255,0.4)"}}>Tagesziel:</span>
            {[1500,1800,2000,2500].map(g=>(
              <button key={g} onClick={()=>setDailyCal(dc=>({...dc,goal:g}))}
                style={{background:dailyCal.goal===g?"linear-gradient(135deg,#ff6b35,#ff9a3c)":"rgba(255,255,255,0.07)",border:"none",borderRadius:"8px",color:"white",padding:"0.25rem 0.5rem",fontSize:"0.7rem",fontWeight:dailyCal.goal===g?"800":"400",cursor:"pointer",fontFamily:"inherit"}}>
                {g}
              </button>
            ))}
          </div>
          {/* Calorie log */}
          {dailyCal.log.length>0&&<div style={{borderTop:"1px solid rgba(255,255,255,0.06)",paddingTop:"0.6rem",maxHeight:"100px",overflowY:"auto"}}>
            {dailyCal.log.slice(-4).reverse().map((l,i)=>(
              <div key={i} style={{display:"flex",justifyContent:"space-between",fontSize:"0.72rem",color:"rgba(255,255,255,0.4)",padding:"0.15rem 0"}}>
                <span>{l.name}</span><span style={{color:"#fb923c"}}>{l.cal} kcal · {l.time}</span>
              </div>
            ))}
          </div>}
          <button onClick={()=>setDailyCal(dc=>({...dc,total:0,log:[]}))} style={{marginTop:"0.6rem",background:"rgba(255,68,68,0.1)",border:"1px solid rgba(255,68,68,0.15)",borderRadius:"10px",color:"rgba(255,120,120,0.7)",padding:"0.35rem 0.8rem",cursor:"pointer",fontFamily:"inherit",fontSize:"0.72rem"}}>Reset</button>
        </div>

        {/* PAAR-MODUS */}
        <div style={{background:"rgba(255,255,255,0.07)",borderRadius:"20px",padding:"1.1rem",marginBottom:"1rem",border:"1px solid rgba(255,255,255,0.08)",backdropFilter:"blur(20px)"}}>
          <div style={{fontSize:"0.62rem",letterSpacing:"2px",color:"rgba(255,107,53,0.9)",marginBottom:"0.6rem"}}>👫 PAAR-MODUS</div>
          <div style={{color:"rgba(255,255,255,0.5)",fontSize:"0.78rem",marginBottom:"0.8rem",lineHeight:1.5}}>Beide swipen auf demselben Gerät. Bei gegenseitigem ❤️ gibt's ein Match!</div>
          <div style={{display:"flex",gap:"0.5rem",marginBottom:"0.7rem"}}>
            <button onClick={()=>{setPaarUser("A");save("fs_paaruser","A");}} style={{flex:1,background:paarUser==="A"?"linear-gradient(135deg,#ff6b35,#ff9a3c)":"rgba(255,255,255,0.07)",border:"none",borderRadius:"12px",color:"white",padding:"0.6rem",fontWeight:paarUser==="A"?"800":"400",cursor:"pointer",fontFamily:"inherit"}}>Person A</button>
            <button onClick={()=>{setPaarUser("B");save("fs_paaruser","B");}} style={{flex:1,background:paarUser==="B"?"linear-gradient(135deg,#8e44ad,#9b59b6)":"rgba(255,255,255,0.07)",border:"none",borderRadius:"12px",color:"white",padding:"0.6rem",fontWeight:paarUser==="B"?"800":"400",cursor:"pointer",fontFamily:"inherit"}}>Person B</button>
          </div>
          <div style={{display:"flex",alignItems:"center",gap:"0.8rem"}}>
            <button onClick={()=>setPaarMode(p=>!p)} style={{flex:1,background:paarMode?"linear-gradient(135deg,#ff6b9d,#ff85b3)":"rgba(255,255,255,0.07)",border:`1px solid ${paarMode?"rgba(255,107,157,0.4)":"rgba(255,255,255,0.1)"}`,borderRadius:"12px",color:"white",padding:"0.7rem",fontWeight:"700",cursor:"pointer",fontFamily:"inherit"}}>
              {paarMode?"💞 Paar-Modus aktiv":"💞 Paar-Modus aktivieren"}
            </button>
          </div>
          {paarMode&&<div style={{marginTop:"0.6rem",fontSize:"0.72rem",color:"rgba(255,255,255,0.35)",textAlign:"center"}}>Aktiv als Person {paarUser} · Gerät wechseln für Person {paarUser==="A"?"B":"A"}</div>}
        </div>

        {/* TASTE PROFILE */}
        {Object.keys(tasteProfile).length>0&&(
          <div style={{background:"rgba(255,255,255,0.07)",borderRadius:"20px",padding:"1.1rem",marginBottom:"1rem",border:"1px solid rgba(255,255,255,0.08)"}}>
            <div style={{fontSize:"0.62rem",letterSpacing:"2px",color:"rgba(255,107,53,0.9)",marginBottom:"0.6rem"}}>🧠 DEIN GESCHMACKSPROFIL</div>
            <div style={{display:"flex",flexWrap:"wrap",gap:"0.4rem"}}>
              {Object.entries(tasteProfile).sort((a,b)=>b[1]-a[1]).slice(0,8).map(([tag,score])=>(
                <div key={tag} style={{background:`rgba(255,107,53,${Math.min(0.5,score*0.12)})`,border:"1px solid rgba(255,107,53,0.25)",borderRadius:"50px",padding:"0.3rem 0.75rem",fontSize:"0.72rem",color:"rgba(255,255,255,0.8)"}}>
                  {tag} <span style={{color:"#ff9a3c",fontWeight:"700"}}>{score>1?`×${score}`:""}</span>
                </div>
              ))}
            </div>
            <button onClick={()=>setTasteProfile({})} style={{marginTop:"0.6rem",background:"rgba(255,68,68,0.1)",border:"1px solid rgba(255,68,68,0.15)",borderRadius:"10px",color:"rgba(255,120,120,0.7)",padding:"0.35rem 0.8rem",cursor:"pointer",fontFamily:"inherit",fontSize:"0.72rem"}}>Profil zurücksetzen</button>
          </div>
        )}
  ...
  if(screen==="weekplan"){
    ...
    const SLOTS=[{id:"breakfast",label:"🌅",short:"Früh"},{id:"lunch",label:"☀️",short:"Mittag"},{id:"dinner",label:"🌙",short:"Abend"}];
    const setWpRecipe=(day,slot,recipe)=>{
      setWeekPlan(p=>({...p,[`${day}_${slot}`]:recipe}));
      setWpSlot(null);
    };
    const genShoppingList=()=>{
      const all=[];
      Object.values(weekPlan).forEach(r=>{
        if(r?.ingredients)r.ingredients.forEach(i=>{
          if(!all.find(x=>x.name===i.name))all.push(i);
        });
      });
      setShoppingList(all);
      showToast("🛒 Einkaufsliste erstellt!");
      setScreen("shopping");
    };
    ...
    // Slot picker overlay
    if(wpSlot){
      const favs=liked.length>0?liked:LOCAL_RECIPES.slice(0,8);
      return(
        <div style={{minHeight:"100dvh",fontFamily:"'DM Sans',sans-serif",color:"white",position:"relative"}}>
          <BG img={current?.img}/>
          <div style={{position:"relative",zIndex:1,padding:"1.2rem 1.4rem"}}>
            <div style={{display:"flex",alignItems:"center",gap:"1rem",marginBottom:"1.2rem"}}>
              <button onClick={()=>setWpSlot(null)} style={{background:"rgba(255,255,255,0.1)",border:"none",borderRadius:"50%",color:"white",width:40,height:40,fontSize:"1.2rem",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0}}>←</button>
              <div>
                <div style={{fontWeight:"800",fontSize:"1rem"}}>{wpSlot.day} – {SLOTS.find(s=>s.id===wpSlot.slot)?.label} {SLOTS.find(s=>s.id===wpSlot.slot)?.short}</div>
                <div style={{color:"rgba(255,255,255,0.4)",fontSize:"0.78rem"}}>Gericht auswählen</div>
              </div>
            </div>
            <div style={{display:"grid",gap:"0.6rem",marginBottom:"1.5rem"}}>
              {favs.map((r,i)=>(
                <div key={i} onClick={()=>setWpRecipe(wpSlot.day,wpSlot.slot,r)}
                  style={{borderRadius:"14px",overflow:"hidden",background:"rgba(255,255,255,0.08)",display:"flex",height:"72px",border:"1px solid rgba(255,255,255,0.1)",cursor:"pointer"}}>
                  <img src={r.img} alt={r.name} style={{width:72,height:72,objectFit:"cover",flexShrink:0}} onError={e=>{e.target.src=FALLBACK;}}/>
                  <div style={{padding:"0.6rem 0.9rem",flex:1,display:"flex",alignItems:"center"}}>
                    <div>
                      <div style={{fontWeight:"700",fontSize:"0.88rem"}}>{r.name}</div>
                      <div style={{color:"rgba(255,255,255,0.4)",fontSize:"0.72rem"}}>🔥 {r.cal} kcal · 💪 {r.protein}g</div>
                    </div>
                  </div>
                </div>
              ))}
            </div>
            {liked.length===0&&<div style={{textAlign:"center",color:"rgba(255,255,255,0.3)",fontSize:"0.82rem",padding:"0 1rem"}}>Tipp: Swipe erst Gerichte nach rechts für mehr Auswahl!</div>}
          </div>
          {FONT}
        </div>
      );
      ...
      <div style={{minHeight:"100dvh",fontFamily:"'DM Sans',sans-serif",color:"white",position:"relative"}}>
        ...
        <div style={{position:"relative",zIndex:1,padding:"1.2rem 1.4rem"}}>
          <div style={{display:"flex",alignItems:"center",gap:"1rem",marginBottom:"0.5rem"}}>
            <BackBtn to="swipe"/>
            <h1 style={{margin:0,fontSize:"1.4rem",fontWeight:"800"}}>Wochenplan 📅</h1>
            {filledCount>0&&<span style={{marginLeft:"auto",background:"rgba(255,107,53,0.3)",borderRadius:"50px",padding:"0.2rem 0.75rem",fontSize:"0.72rem",fontWeight:"700",color:"#ff9a3c"}}>{filledCount}/21</span>}
          </div>
          <div style={{color:"rgba(255,255,255,0.35)",fontSize:"0.78rem",marginBottom:"1.2rem"}}>Tippe auf einen Slot um ein Gericht zuzuweisen</div>

          {DAYS.map(day=>(
            <div key={day} style={{marginBottom:"0.8rem",background:"rgba(255,255,255,0.05)",borderRadius:"16px",overflow:"hidden",border:"1px solid rgba(255,255,255,0.08)"}}>
              <div style={{padding:"0.55rem 0.9rem",background:"rgba(255,255,255,0.07)",fontSize:"0.78rem",fontWeight:"800",letterSpacing:"0.5px",color:"rgba(255,255,255,0.7)"}}>{day}</div>
              <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"1px",background:"rgba(255,255,255,0.06)"}}>
                {SLOTS.map(({id,label,short})=>{
                  const r=weekPlan[`${day}_${id}`];
                  return(
                    <div key={id} onClick={()=>setWpSlot({day,slot:id})}
                      style={{background:"rgba(8,8,18,0.9)",padding:"0.5rem 0.4rem",cursor:"pointer",minHeight:"60px",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:"0.2rem",position:"relative",overflow:"hidden"}}>
                      {r?(
                        <>
                          <img src={r.img} alt="" style={{position:"absolute",inset:0,width:"100%",height:"100%",objectFit:"cover",opacity:0.3}} onError={e=>{e.target.src=FALLBACK;}}/>
                          <div style={{position:"relative",fontSize:"0.62rem",fontWeight:"700",textAlign:"center",lineHeight:1.2,zIndex:1}}>{r.name.length>14?r.name.slice(0,13)+"…":r.name}</div>
                          <div style={{position:"relative",fontSize:"0.55rem",color:"rgba(255,255,255,0.5)",zIndex:1}}>🔥{r.cal}</div>
                        </>
                      ):(
                        <>
                          <div style={{fontSize:"1rem",opacity:0.4}}>{label}</div>
                          <div style={{fontSize:"0.6rem",color:"rgba(255,255,255,0.3)"}}>{short}</div>
                          <div style={{fontSize:"0.6rem",color:"rgba(255,107,53,0.5)"}}>+ Hinzufügen</div>
                        </>
                      )}
                    </div>
                  );
          ...
          <div style={{display:"flex",gap:"0.6rem",marginTop:"0.5rem",marginBottom:"2rem"}}>
            <button onClick={genShoppingList} disabled={filledCount===0}
              style={{flex:1,background:filledCount>0?"linear-gradient(135deg,#ff6b35,#ff9a3c)":"rgba(255,255,255,0.05)",border:"none",borderRadius:"14px",color:"white",padding:"1rem",fontWeight:"800",cursor:filledCount>0?"pointer":"default",fontFamily:"inherit",opacity:filledCount>0?1:0.4,fontSize:"0.9rem"}}>
              🛒 Einkaufsliste generieren
            </button>
            <button onClick={()=>setWeekPlan({})} style={{background:"rgba(255,68,68,0.1)",border:"1px solid rgba(255,68,68,0.2)",borderRadius:"14px",color:"rgba(255,120,120,0.7)",padding:"1rem",cursor:"pointer",fontFamily:"inherit"}}>🗑</button>
          </div>
        </div>
        {FONT}
      </div>
      ...
      <div style={{position:"relative",zIndex:1,display:"flex",flexDirection:"column",height:"100%"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"1rem 1.2rem 0.2rem"}}>
          ...
          <div style={{textAlign:"center"}}>
            ...
            {dailyCal.total>0&&<div style={{fontSize:"0.62rem",color:"rgba(255,255,255,0.35)",marginTop:"0.1rem"}}>🔥 {dailyCal.total}/{dailyCal.goal} kcal heute</div>}
          </div>
          <div style={{display:"flex",gap:"0.5rem"}}>
            ...
            <button onClick={()=>setScreen("weekplan")} style={{background:"rgba(255,255,255,0.12)",backdropFilter:"blur(10px)",border:"1px solid rgba(255,255,255,0.12)",borderRadius:"50px",color:"white",padding:"0.4rem 0.85rem",cursor:"pointer",fontSize:"0.95rem"}}>📅</button>
        ...
        <div style={{padding:"0.3rem 1rem 0.35rem",display:"flex",gap:"0.4rem",overflowX:"auto",scrollbarWidth:"none",flexShrink:0}}>
          {MOODS.map(m=>(
            <button key={m} onClick={()=>{setMoodFilter(m);applyFilter(m,selectedAllergens);}}
              ...
              {m==="Zur Uhrzeit"?`${slotInfo.emoji} ${m}`:m==="Saisonal"?`${SEASON_LABEL()}`:m}
            </button>
          ))}
